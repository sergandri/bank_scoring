...
class FeatureSelector:
    def __init__(self, analyser: FeatureAnalyser):
    ...
    def calculate_correlation(self, threshold: float = 0.7):
        ...
        for column in upper_triangle.columns:
            ...
            #    continue
            self.selected_features_corr.append(column)

        corr_results_df = corr_matrix.loc[
            self.selected_features_corr, self.selected_features_corr]
        corr_results_df.to_csv(
            "correlation_filtered_features.csv", index_label='Feature'
            )
